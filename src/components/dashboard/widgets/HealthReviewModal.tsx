...
export function HealthReviewModal({ report: initialReport, checkName, onClose, onResolved }: Props) {
   ...
   */
  const pruneItem = useCallback((check: string, id: string, extras?: { raw?: string }) => {
    setReport(prev => {
      ...
      switch (check) {
        ...
        case 'broken_internal_links': {
          // Pair-level prune. `extras.raw` identifies the specific link
          // text just resolved; only drop that entry. An atom with multiple
          // distinct broken targets must stay in the list so the remaining
          // rows render. Dropping the whole atom here and then refetching
          // the server report makes the row flicker out and back.
          const atoms = data.broken_link_list as
            | Array<{ atom_id: string; atom_title: string; links: Array<{ raw: string; target: string; kind: string }> }>
            | undefined;
          if (!atoms) break;
          let nextAtoms = atoms;
          ...
          if (extras?.raw) {
            nextAtoms = atoms
              .map(a => {
                if (a.atom_id !== id) return a;
                const beforeLen = a.links.length;
                const filtered = a.links.filter(l => l.raw !== extras.raw);
                removedLinkCount += beforeLen - filtered.length;
                return { ...a, links: filtered };
              })
              .filter(a => a.links.length > 0);
          } else {
            // No raw supplied — treat as "whole atom resolved" (auto-fix-all,
            // dismiss). Subtract every remaining link on this atom.
            const match = atoms.find(a => a.atom_id === id);
            removedLinkCount = match?.links.length ?? 0;
            nextAtoms = atoms.filter(a => a.atom_id !== id);
          }
          if (nextAtoms !== atoms && (nextAtoms.length !== atoms.length || removedLinkCount > 0)) {
            touched = true;
            data.broken_link_list = nextAtoms;
            // Affected-atoms count mirrors nextAtoms.length (atoms that still
            // have any broken link).
            if (typeof data.affected_atoms === 'number') {
              data.affected_atoms = nextAtoms.length;
            }
            if (typeof data.broken_count === 'number') {
              data.broken_count = Math.max(0, (data.broken_count as number) - removedLinkCount);
            }
            // Legacy key — kept for older summary consumers.
            if (typeof data.broken_links === 'number') {
              data.broken_links = Math.max(0, (data.broken_links as number) - removedLinkCount);
            }
          }
          break;
        }
  ...
  const resolveItem = useCallback((check: string, id: string, extras?: { raw?: string }) => {
    pruneItem(check, id, extras);
    bumpResolved(check);
    onResolved();
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-3">
          ...
          {activeTab === 'broken_internal_links' && brokenLinksData && (
            <>
              ...
              <BrokenLinksSection data={brokenLinksData as { broken_link_list: import('./review/BrokenLinksSection').BrokenLinkAtom[] }} onResolved={(id, raw) => resolveItem('broken_internal_links', id, raw ? { raw } : undefined)} />
            </>
          )}
