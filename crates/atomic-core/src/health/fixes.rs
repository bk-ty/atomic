...
pub async fn remove_broken_link(
    ...
    // Replace every occurrence of the broken link. Atoms with the same link
    // repeated (e.g. a linked term used throughout a paragraph) would
    // otherwise leak back into the next scan as a still-broken occurrence,
    // forcing the user to click Remove once per instance.
    let occurrences = content.matches(link_raw).count();
    let new_content = content.replace(link_raw, &replacement);
    ...
    Ok(FixAction {
        ...
        count: occurrences as i32,
        details: vec![format!(
            "Removed link '{}' from atom {} ({} occurrence{})",
            link_raw,
            atom_id,
            occurrences,
...
pub async fn relink_broken_link(
    ...
    // Relink every occurrence of this broken link in the atom. A term
    // linked repeatedly (e.g. "Glossary" referenced five times) would
    // otherwise need five separate Relink clicks.
    let occurrences = content.matches(link_raw).count();
    let new_content = content.replace(link_raw, &new_link);
    ...
    Ok(FixAction {
        ...
        count: occurrences as i32,
        details: vec![format!(
            "Relinked '{}' in atom {} → atom://{} ('{}') × {} occurrence{}",
            link_raw,
            atom_id,
            target_atom_id,
            target_title,
