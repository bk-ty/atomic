...
mod tests {
    ...
    fn test_content_quality_no_source_atoms() {
        let mut raw = base_raw();
        raw.no_source_atoms.push(AtomPreview { id: "atom-1".to_string(), title: "My Note".to_string(), created_at: "2026-01-01T00:00:00Z".to_string(), suggested_source: None });
        raw.no_source_atoms.push(AtomPreview { id: "atom-2".to_string(), title: "Another Note".to_string(), created_at: "2026-01-02T00:00:00Z".to_string(), suggested_source: None });
        let result = checks::content_quality(&raw);
        assert!(result.requires_review);
        // Check data shape
