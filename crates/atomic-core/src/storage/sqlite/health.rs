...
impl SqliteStorage {
    ...
    pub(crate) fn health_check_data_impl(
        ...
        while let Some(row) = rows.next()? {
            ...
            raw.no_source_atoms.push(crate::health::AtomPreview {
                id,
                title,
                created_at,
        ...
        // For each no-source atom, try to find a matching orphaned feed item
        // (atom_id IS NULL means the original atom was deleted) so we can suggest
        // the original source URL.  Matching is purely local: normalised title
        // token overlap ≥ 0.5, no network calls required.
        {
            // Collect all orphaned feed items that have both title and link stored.
            let mut fi_stmt = conn.prepare(
                "SELECT item_title, item_link FROM feed_items
                 WHERE atom_id IS NULL AND skipped = 0
                   AND item_title IS NOT NULL AND item_link IS NOT NULL",
            )?;
            let orphaned: Vec<(String, String)> = fi_stmt
                .query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))?
                .filter_map(|r| r.ok())
                .collect();

            if !orphaned.is_empty() {
                for preview in &mut raw.no_source_atoms {
                    if preview.suggested_source.is_some() {
                        continue;
                    }
                    // Use token-overlap to find the best matching feed item.
                    // Threshold 0.5 — requires half the title tokens to match.
                    let best = orphaned.iter().find(|(fi_title, _)| {
                        titles_share_token(&preview.title, fi_title)
                            || content_token_jaccard(&preview.title, fi_title) >= 0.5
                    });
                    if let Some((_, link)) = best {
                        preview.suggested_source = Some(link.clone());
                    }
                }
            }
        }
